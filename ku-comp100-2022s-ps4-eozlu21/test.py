...
def test_p4_3():
    crobat = EvolvedPokemon('Crobat', 'Psychic', 1000, 130, ('Lightning', 2), 50, 10)
    crobat.add_power('Ultra-Toxic Fang')  # 40

    pikachu = Pokemon(name='pikachu', energy_type='Lightning', cost=100, hp=60, weakness=('Fighting', 2))
    pikachu.add_power('Quick Attack')  # 10
    pikachu.add_power('Electro Ball')  # 50

    pikachu.battle_step(crobat)
    assert not pikachu.is_healthy()
    assert crobat.get_hp() == 20
    assert pikachu.get_num_wins() == 0
    assert crobat.get_num_wins() == 0, "num wins should only update during battle, not battle step."

    print("Passed test 4.3")

def test_p4_4(): # same as p4_3(), but uses battle()
    crobat = EvolvedPokemon('Crobat', 'Psychic', 1000, 130, ('Lightning', 2), 50, 10)
    crobat.add_power('Ultra-Toxic Fang')  # 40
    ...
    pikachu.battle(crobat)
    assert not pikachu.is_healthy()
    assert crobat.get_hp() == 20
    assert pikachu.get_num_wins() == 0
    assert crobat.get_num_wins() == 1

    print("Passed test 4.4")
...
def test_p4_5():
    ...
    raichu = EvolvedPokemon(name='Raichu', energy_type='Lightning', cost=800, hp=90, weakness=('Fighting', 2),
                            damage_booster=30, damage_reducer=0)
    raichu.add_power('Quick Attack')  # 10
    raichu.add_power('Electro Ball')  # 50

    raichu.battle_step(crobat)
    assert not raichu.is_healthy()
    assert not crobat.is_healthy()
    assert raichu.get_num_wins() == 0
    assert crobat.get_num_wins() == 0

    print("Passed test 4.5")

def test_p4_6(): # same as p4_6(), but uses battle()
    crobat = EvolvedPokemon('Crobat', 'Psychic', 1000, 130, ('Lightning', 2), 50, 10)
    crobat.add_power('Ultra-Toxic Fang')  # 40
...
def test_p5_1(): # testing trainer fight
    ...
    assert len(gary.get_pokemons()) == 1

    assert not raichu.is_healthy()
    assert raichu.get_num_wins() == 0
    assert raichu.get_taken()
    assert len(ash.get_pokemons()) == 0
    print("Passed test 5.1")

def test_p5_2():
    ash = Trainer('Ash', 1000)
    gary = Trainer('Gary', 1000)
    raichu = EvolvedPokemon(name='Raichu', energy_type='Lightning', cost=1200, hp=90, weakness=('Fighting', 1.2),
                            damage_booster=30, damage_reducer=0)
    terrakion = Pokemon('Terrakion', 'Fighting', 200, 130, ('Grass', 2))

    ash.add_pokemon(raichu) # should fail because of cost
    assert raichu not in ash.get_pokemons()
    assert not raichu.get_taken()
    assert ash.get_xp() == 1000
    ash.add_pokemon(terrakion) # should succeed
    assert terrakion in ash.get_pokemons()
    assert terrakion.get_taken()
    assert ash.get_xp() == 800
    gary.add_pokemon(terrakion) # should fail because terrakion is taken
    assert terrakion not in gary.get_pokemons()
    assert terrakion.get_taken()
    assert gary.get_xp() == 1000

    print("Passed test 5.2")
...
def test_p5_3():
    ...
    assert raichu.get_num_wins() == 0
    assert crobat.get_num_wins() == 0
    assert len(gary.get_pokemons()) == 0 # because both pokemon can be knocked out simultaneously
    assert len(ash.get_pokemons()) == 0

    print("Passed test 5.3")

def test_p6_1():

    crobat = EvolvedPokemon('Crobat', 'Psychic', 1000, 130, ('Lightning', 2), 50, 10)
    crobat.add_power('Ultra-Toxic Fang')  # 40
    ash = Trainer('Ash', 1200)
    ash.add_pokemon(crobat)

    landorus = EvolvedPokemon('Landorus', 'Fighting', 400, 110, ('Psychic', 3), 20, 20)
    landorus.add_power('Smack')  # 10
    landorus.add_power('Acrobatics')  # 40
    meloetta = Pokemon('Meloetta', 'Fighting', 100, 80, ('Psychic', 3))
    meloetta.add_power('Smack')  # 10
    meloetta.add_power('Acrobatics')  # 40
    gary = Trainer('Gary', 10000)
    gary.add_pokemon(landorus)
    gary.add_pokemon(meloetta)

    ash.fight(gary)
    assert gary.are_all_defeated()
    assert len(gary.get_pokemons()) == 0
    assert not ash.are_all_defeated()
    assert crobat.get_num_wins() == 2
    assert crobat.is_healthy()

    print("Passed test 6.1")
